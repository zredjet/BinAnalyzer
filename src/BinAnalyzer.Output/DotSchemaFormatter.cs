...
public class DotSchemaFormatter : ISchemaFormatter
{
    public string Format(FormatDefinition format)
    {
        ...
        sb.AppendLine("    node [shape=record, fontname=\"Helvetica\", fontsize=10];");
        ...
        foreach (var (name, structDef) in format.Structs)
        {
            ...
            var fieldsStr = BuildFieldList(structDef);
            var label = $"{{{EscapeDot(name)}|{fieldsStr}}}";

            if (name == format.RootStruct)
                sb.AppendLine($"    {safeName} [label=\"{label}\", style=bold, color=blue];");
            else
                sb.AppendLine($"    {safeName} [label=\"{label}\"];");
        }
    ...
    private static string BuildFieldList(StructDefinition structDef)
    {
        var sb = new StringBuilder();
        foreach (var field in structDef.Fields)
        {
            var typeStr = GetFieldTypeDisplay(field);
            sb.Append($"{EscapeDot(field.Name)} : {typeStr}\\l");
        }
        return sb.ToString();
    }
    ...
    private static List<string> CollectEdges(string fromNode, FieldDefinition field)
    {
        ...
        if (field.Type == FieldType.Switch)
        {
            if (field.SwitchCases is { } cases)
            {
                foreach (var sc in cases)
                {
                    ...
                    var label = EscapeDot($"{field.Name} [{condText}]{multiplicity}");
                    edges.Add($"    {fromNode} -> {targetNode} [label=\"{label}\"];");
                }
            }
            ...
            if (field.SwitchDefault is { } defaultRef)
            {
                ...
                var label = EscapeDot($"{field.Name} [default]{multiplicity}");
                edges.Add($"    {fromNode} -> {targetNode} [label=\"{label}\"];");
            }
        }
        else if (field.StructRef is { } structRef)
        {
            ...
            var label = EscapeDot($"{field.Name}{compression}{multiplicity}");
            edges.Add($"    {fromNode} -> {targetNode} [label=\"{label}\"];");
        }
