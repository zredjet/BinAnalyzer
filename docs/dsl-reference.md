...
式が定義のどこにも無い名前（どの struct のフィールド・bitfield のエントリ・テンプレートのパラメータでもなく、`_index` / `_prev` / `remaining` でもない名前）を参照していると、検証で警告 VAL124 になります（`true` / `false` には 1 / 0 を案内し、それ以外は近い名前を候補に示します）。組み込み関数に無い関数は VAL125 です。入れ子の struct の値は親のスコープへ昇格するので、検証は定義全体の名前で判定します。定義のどこかにはあるがその式からは見えない名前（まだデコードしていない後ろのフィールドなど）は検証では見つからず、デコード時のエラーになります。
