...
| `uint64` / `u64` | 8バイト | 符号なし64ビット整数（2^63 以上も正の値で表示。式の中では符号付き。下の「整数の値の範囲」） |
| `int8` / `i8` | 1バイト | 符号付き8ビット整数 |
| `int16` / `i16` | 2バイト | 符号付き16ビット整数 |
| `int32` / `i32` | 4バイト | 符号付き32ビット整数 |
| `int64` / `i64` | 8バイト | 符号付き64ビット整数 |
| `float16` / `f16` | 2バイト | IEEE 754 半精度浮動小数点数（binary16。値は倍精度に広げて表示。最大 65504） |
...
- **`_offset`**: `{_offset}` — 今の読み取り位置（今のデータの先頭からのバイト数。`seek:` と同じ基準）。size 付きのスコープの中でもファイルの先頭からの位置になる（`remaining` はスコープの残り）。`seek: "{_offset}"` + `seek_restore: true` で今の位置のバイトを別の型で読み直す・先を覗く、直前に virtual で取っておいてポインタの基準にする、などに使う。展開した圧縮データの中では展開した中身の先頭から、ビットストリームモードの中では読みかけのバイトの位置（端数のビットは切り捨て）（REQ-194）
- **括弧**: `{(a + b) * c}`
- **関数呼び出し**: `{func_name(arg1, arg2, ...)}` — 組み込み関数の呼び出し

真偽値のリテラル（`true` / `false`）はありません。式の中の識別子はすべてフィールドなどの名前として引かれるので、`{a ? b : false}` は `false` という名前のフィールドを探してデコードエラーになります。真は `1`、偽は `0` と書きます（比較・論理演算の結果はそのまま条件に使えます）。

式が定義のどこにも無い名前（どの struct のフィールド・bitfield のエントリ・テンプレートのパラメータでもなく、`_index` / `_prev` / `remaining` / `_offset` でもない名前）を参照していると、検証で警告 VAL124 になります（`true` / `false` には 1 / 0 を案内し、それ以外は近い名前を候補に示します）。組み込み関数に無い関数は VAL125 です。入れ子の struct の値は親のスコープへ昇格するので、検証は定義全体の名前で判定します。定義のどこかにはあるがその式からは見えない名前（まだデコードしていない後ろのフィールドなど）は検証では見つからず、デコード時のエラーになります。

### 整数の値の範囲

式の整数は符号付き 64 ビット（-2^63 〜 2^63 - 1）です（REQ-201）。符号なし 64 ビットの型（`uint64`・`uleb128`・`vlq`）の 2^63 以上の値は、ツリー・JSON・CSV・GUI / TUI の詳細では符号なし（`18446744073709551615`、16 進は `0xFFFFFFFFFFFFFFFF`）で表示しますが、**式の中では同じビットの負の値**になります。

- 比較は符号付き: `uint64` の 0xFFFFFFFFFFFFFFFF は式の中では -1 なので、`{x > 0}` は偽になる
- 算術は 2^64 で割った余り（`+`・`-`・`*` はビットの並びとしては符号なしと同じ結果になる。`/`・`%`・`>>` は符号付きとして計算する）
- virtual の値は式の結果なので、2^63 以上になる値は負の値で表示される。値そのものを見せたいときは、元の `uint64` のフィールドを表示するか、`seek` + `seek_restore` で `uint64` として読み直す（formats/cbor.bdef.yaml の 8 バイトの引数の符号なし整数）
- JSON の `value` は数値で書く（2^53 より大きい値は JavaScript の Number では精度が落ちるので、正確な値は `hex` を使う）
...
`value` は 10 進（負号可）か `0x` の 16 進の整数で、-2^63 〜 2^64 - 1 を書けます。2^63 以上の値（`0xFFFFFFFFFFFFFFFF` など）は `uint64` のフィールドの値と照合します（REQ-201）。
...
- `repeat_until`: 要素をデコードした**後**に条件評価。条件が真になった要素も配列に含まれる。要素が独自の変数のスコープを持つ struct（`scope: isolated`・エンディアンを書いた形・テンプレートの引数）でも、要素の値（繰り返しの後ろに昇格する値と同じ範囲）を条件から引ける（REQ-196）
...
要素が 0 バイトで成功する（virtual だけの struct、`if:` が偽のフィールドだけの struct、残りを全部読んだ後の `size: remaining` など）と、位置が進まないまま繰り返しが続きます。壊れた要素数（数十億）でも終わるよう、**0 バイトの要素が 65536 個続いたら** 繰り返しを打ち切ります（REQ-197）。全繰り返しモード（`repeat_count`・`repeat: eof`・`repeat_until`・`repeat_while`）で働き、1 バイト以上の要素で数え直します。要素ごとの seek（`_index` を使う seek）の繰り返しは、位置が要素ごとに決まるので対象外です。打ち切った配列は `(truncated)` と理由を持ちます。上限は API の `DecodeOptions.MaxZeroLengthElements` で変えられます。
