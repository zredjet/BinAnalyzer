...
- VAL124: 式が定義のどこにも無い名前を参照している（全 struct のフィールド・bitfield のエントリ・テンプレートのパラメータと `_index` / `_prev` / `remaining` のどれでもない。`true` / `false` には 1 / 0 を案内し、それ以外は近い名前を候補に示す。値の昇格があるので定義全体の名前で近似しており、定義にはあるがその式からは見えない名前は見つけられない）
