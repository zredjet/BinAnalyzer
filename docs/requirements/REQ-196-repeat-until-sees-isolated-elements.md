...
- [ ] **`repeat_until` の条件で、独自のスコープを持つ struct の要素の値を引けるようにする。** 要素の直下のフィールドの値（昇格する値と同じ範囲）が条件から見えること
- [ ] `element_size` の要素（今でも要素のスコープの中で条件を評価している）の動作は変えない
...
1. [ ] エンディアンの形の struct を要素にした `repeat_until` で、要素のフィールドの値を条件に使えること（CBOR の break と同じ形）
2. [ ] テンプレートの引数を持つ struct の要素でも同じであること
3. [ ] 大きさの無い struct・`element_size` の要素の `repeat_until` の動作が変わらないこと
4. [ ] 条件で引けた値が、繰り返しの後ろ（昇格した値）と一致すること
...
| tests/BinAnalyzer.Engine.Tests | 独自のスコープの要素の `repeat_until` のテスト |

### 変更が必要なドキュメント

- [ ] docs/dsl-reference.md — 「条件成立まで繰り返し」
