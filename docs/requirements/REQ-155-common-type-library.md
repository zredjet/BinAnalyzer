...
- 共通ライブラリファイルは単体では `validate` が通らない（`container_box` が import 元の `iso_box` を参照するため VAL002）。REQ-178 で扱う
- REQ-126（import 実用化）は本要望で中核が実現されたため取り下げ。残件の DOS datetime 共通化は REQ-179
