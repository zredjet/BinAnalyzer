...
- 見直しの中で DSL の制約のために残した「対応していないもの」のうち、次を要望として起こした: pcap の中の DNS → REQ-198、インターネットチェックサム・Ogg の CRC-32・OTF の表のチェックサム → REQ-199、半精度の浮動小数点数（CBOR）→ REQ-200、2^63 以上の uint64（MessagePack・CBOR）→ REQ-201
